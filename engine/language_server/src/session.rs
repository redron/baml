...
impl Session {
    ...
    pub(crate) fn default_project_db(&self) -> Option<&Project> {
        // SAFETY: Currently, red knot only support a single project.
        self.projects_by_workspace_folder.values().next()
    }

    /// Returns a mutable reference to the default project [`ProjectDatabase`].
    pub(crate) fn default_project_db_mut(&mut self) -> Option<&mut Project> {
        // SAFETY: Currently, red knot only support a single project.
